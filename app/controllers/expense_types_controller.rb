  ...
  def create
    ...
    respond_to do |format|
      if @expense_type.save
        ...
        format.xml  { render :xml => expense_types_url, :status => :created, :location => @expense_type }
      else
  ...
  def update
    ...
    respond_to do |format|
      if @expense_type.update_attributes(params[:expense_type])
        format.html { redirect_to(expense_types_url, :notice => 'Expense type was successfully updated.') }
        format.xml  { head :ok }
      else
